...
const VideoHeaderCard = () => {
  ...
  return (
    <StyledMainContainer width={width}>
      <Header width={width}>
        {width >= 768 && (
          <StyledBtn1 to={"/"} type="text" shape="circle">
            {" "}
            <ArrowLeftOutlined />{" "}
          </StyledBtn1>
        )}
        ...
        <Content>
          <div>
            <Typography.Title level={3} style={{ minWidth: "125px" }}>
              {" "}
              What is CSS Flexbox?
            </Typography.Title>
          </div>
        ...
        <Space size={12} className="space-antd-btn" wrap={true}>
          <StyledBtn2 type="text" size="large">
            Previous Lecture
          </StyledBtn2>
          <StyledBtn2 type="text" size="large">
            Next Lecture
...
const StyledMainContainer = styled.div`
  ...
  box-shadow: ${(props) =>
    props.width >= 768 ? "0px 0px 3px 0px rgba(0, 0, 0, 0.25)" : "none"};
`;

const StyledBtn1 = styled(Link)`
  font-size: 1.5rem;
  height: 50px;
  width: 50px;
  min-width: auto;
